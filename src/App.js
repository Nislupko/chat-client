...
class App extends Component {
    ...
    onEntryHandler = (room) => {
        ...
        this.state.socket.emit('new_entry',JSON.stringify({user:this.state.currentUser, room:room}))
    }
    onExitHandler = () => {
        this.state.socket.emit('exit_room',JSON.stringify({user:this.state.currentUser, room:this.state.currentRoom}))
        this.setState({
            currentRoom:null,
            messages:[],
            users:[]
    ...
    onMessageHandler =(text) =>{
        this.state.socket.emit('create_message',JSON.stringify(
    ...
    return (
            <Layout>
                <div className={styles.menu}>
                    ...
                    <Switch>
                        <Route path="/" exact render={()=><Auth handler={this.onInputHandler}
                                                                user={this.state.currentUser}
                                                                errorMessage={this.state.errorMessage}/>} />
                        <Route path="/rooms/:id" exact render = {(props) => <Room {...props}
                                                                                  params={this.state}
                                                                                  onMessageHandler={this.onMessageHandler}
                                                                                  onEntryHandler={this.onEntryHandler}
                                                                                  onExitHandler={this.onExitHandler}/>} />
                        <Route path="/rooms" exact render = {() => <Lobby rooms={this.state.rooms} handler={this.onNewRoom}/>} />
                    </Switch>
            </Layout>
    );
  }
